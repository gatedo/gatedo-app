...
let PetsController = class PetsController {
    constructor(prisma, cloudflare, gamif) {
        this.prisma = prisma;
    ...
    async update(id, files, body) {
        ...
        const oldPet = await this.prisma.pet.findUnique({
            where: { id },
            select: { photoUrl: true, breed: true, weight: true, bio: true, deathCause: true, ownerId: true },
        });
        const updated = await this.prisma.pet.update({
            where: { id },
            data: dataToUpdate,
        });
        if (dataToUpdate.deathCause && !oldPet?.deathCause && oldPet?.ownerId) {
            this.gamif.onMemorialRegistered(oldPet.ownerId).catch(() => { });
        }
        const wasIncomplete = !(oldPet?.photoUrl && oldPet?.breed && oldPet?.weight && oldPet?.bio);
        const isNowComplete = !!(updated.photoUrl && updated.breed && updated.weight && updated.bio);
        if (wasIncomplete && isNowComplete && oldPet?.ownerId) {
            this.gamif.onProfileComplete(oldPet.ownerId).catch(() => { });
        }
        return updated;
    }
