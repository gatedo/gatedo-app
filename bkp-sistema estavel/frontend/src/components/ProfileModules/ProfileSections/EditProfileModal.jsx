        ...
        <div className="space-y-6">
          ...
          <section className="bg-gray-50 p-6 rounded-[32px] flex items-center gap-6 border border-gray-100">
            ...
            <div className="flex-1">
              <span className="text-[9px] font-black uppercase text-gray-400 block mb-2 tracking-widest">Tema Principal</span>
              <div className="flex gap-2.5 flex-wrap">
                {THEME_COLORS.map(c => (
                  <button key={c.hex} onClick={() => setFormData({...formData, themeColor: c.hex})} className={`w-7 h-7 rounded-full border-2 transition-all ${formData.themeColor === c.hex ? 'border-indigo-600 scale-110' : 'border-transparent'} ${c.hex}`} />
                ))}
              </div>
            </div>
          </section>
