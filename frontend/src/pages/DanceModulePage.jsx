...
// Guard: caso studioPromptLibrary.js ainda não tenha sido atualizado no projeto,
// evita tela branca — os valores abaixo são os defaults do módulo.
const MODULE_KEY = 'dance';
const MOD = MODULE_CONFIG?.[MODULE_KEY] ?? {
  gptsCost: 30, xptReward: 10, xpgReward: 10,
...
const GPTS_COST  = MOD.gptsCost;   // 30
const XPT_REWARD = MOD.xptReward;  // 10
const XPG_REWARD = MOD.xpgReward;  // 10
...
function UploadSlot({ label, icon: Icon, previewUrl, onFile, uploading, uploadPct, accent }) {
  ...
  return (
    ...
    >
      {previewUrl ? (
        <>
          <img src={previewUrl} alt={label} className="w-full object-cover rounded-[22px]"
            style={{ minHeight: 200, maxHeight: 260 }} />
          <div className="absolute inset-0 rounded-[22px] flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-200"
...
function PetSelector({ pets, selected, onSelect }) {
  ...
  return (
    <div>
      <p className="text-[9px] font-black uppercase tracking-[3px] text-white/35 mb-2.5">Escolher gato</p>
        ...
        {pets.map(pet => {
          ...
          return (
            <motion.button key={pet.id} whileTap={{ scale: 0.92 }}
              ...
              style={{
                background: active ? `${C.pink}22` : 'rgba(255,255,255,0.05)',
    ...
    <motion.button whileTap={{ scale: 0.94 }} whileHover={{ scale: 1.02 }}
      ...
      {active && (
        <div className="absolute top-0 right-0 w-14 h-14 rounded-full pointer-events-none"
          style={{ background: `radial-gradient(circle, ${color}50 0%, transparent 70%)`, transform: 'translate(30%,-30%)' }} />
      )}
...
export default function DanceModulePage() {
  ...
  const handleGenerate = async () => {
    ...
    const STEPS = [
      'Analisando a foto do gato...',
      'Criando animação base...',
      'Sincronizando com o beat...',
    ...
    try {
      ...
      const finalUrl =
        creation?.resultUrl || creation?.videoUrl ||
        creation?.previewUrl || creation?.outputVideoUrl ||
        res.data?.resultUrl || res.data?.videoUrl || '';

      if (!finalUrl) throw new Error(res.data?.message || 'A IA não retornou o vídeo.');
      ...
      setCreationObj({
        ...
        videoUrl:    creation?.videoUrl   || finalUrl,
      ...
      console.error('[Dance] generate error:', err);
      setError(err?.response?.data?.message || err?.message || 'Erro ao gerar o vídeo.');
    } finally {
  ...
  const handlePublish = () => {
    ...
    navigate('/comunigato/novo', {
      state: {
        ...
        videoUrl:               creationObj.videoUrl   || resultUrl,
        source:                 'studio',
        moduleKey:              MODULE_KEY,
  ...
  const handleDownload = () => {
    ...
    a.href = resultUrl;
    a.download = `dancinha-${Date.now()}.${isVideoUrl(resultUrl) ? 'mp4' : 'jpg'}`;
    a.target = '_blank';
    a.click();
  };
      ...
      <div className="sticky top-0 z-40 px-4 pt-5 pb-3"
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex-1 text-center">
            <p className="text-[8px] font-black uppercase tracking-[3px] text-white/40">Vídeo IA</p>
            <p className="text-sm font-black text-white">Dancinhas</p>
          </div>
      ...
      <div className="px-4 pt-2 pb-4 space-y-4 max-w-xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10 p-5">
            <span className="text-[8px] font-black uppercase tracking-[3px] text-white/60 block mb-1">Vídeo IA</span>
            <h1 className="text-xl font-black text-white leading-tight mb-0.5">💃 Dancinhas</h1>
        ...
        <div>
          ...
          <UploadSlot
            ...
            uploading={catUploading} uploadPct={catUploadPct}
            accent={C.pink}
          />
          <div className="flex items-center gap-1.5 mt-2 px-1">
            ...
            <span className="text-[9px] font-bold"
              ...
              {catUrl ? 'Foto enviada ✓' : catUploading ? `${catUploadPct}%` : 'Aguardando foto'}
            </span>
          </div>
          <p className="text-[9px] text-white/18 font-medium mt-1.5 text-center">
            Use fotos com corpo visível — fundo limpo melhora a animação
          </p>
        </div>

        {/* Presets */}
        <div>
          <p className="text-[9px] font-black uppercase tracking-[3px] text-white/35 mb-2.5">Estilo da dancinha</p>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={`Ex: fundo de festa néon com confetes, estilo ${activePreset.label.toLowerCase()}...`}
        ...
        <motion.button
          ...
          style={{
            ...
            boxShadow: canGenerate ? '0 8px 28px rgba(236,72,153,0.35)' : 'none',
          ...
          <span className="relative z-10 flex items-center justify-center gap-2">
            {isGenerating ? (
              ...
              <><Sparkles size={16} /> Criar vídeo — {effectiveCost} GPTS</>
            )}
          </span>
        </motion.button>
          ...
          {resultUrl && (
            <motion.div
              ...
              <div className="relative">
                {/* Renderiza vídeo se URL for de vídeo, caso contrário imagem/gif */}
                {isVideoUrl(resultUrl) ? (
                  ...
                  <img src={resultUrl} alt="Vídeo gerado" className="w-full object-cover" />
                )}
                ...
                {isVideoUrl(resultUrl) && (
                  <div className="absolute bottom-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full"
                    ...
                    <span className="text-[9px] font-black text-white">Reproduzindo</span>
                  </div>
                )}
              </div>
              <div className="p-4 space-y-2" style={{ background: 'rgba(26,16,48,0.95)' }}>
                <div className="flex items-center gap-1.5 mb-3">
                  ...
                  <p className="text-xs font-black text-white">Vídeo criado com sucesso!</p>
                </div>
                <motion.button whileTap={{ scale: 0.96 }} onClick={handlePublish}
                  ...
                  style={{ background: GRADIENT, boxShadow: '0 6px 20px rgba(236,72,153,0.3)' }}>
