...
import {
  ...
  AlertTriangle, Coins, Zap, PawPrint, User2,
  Download, RefreshCw, ChevronRight, Globe2,
...
// ─── UploadSlot — div (não label) para evitar disparo duplo do input ─────────
    ...
    >
      {previewUrl ? (
        <>
          ...
          {/* Upload progress overlay on replacement */}
          {uploading && (
        ...
        <div className="flex flex-col items-center gap-2 px-4 py-6 text-center pointer-events-none">
          {uploading ? (
            ...
            <>
              <div className="w-14 h-14 rounded-[20px] flex items-center justify-center mb-1"
      ...
      {/* Input hidden — ref only, sem estar dentro de label */}
      <input
        ref={inputRef}
        type="file"
        accept="image/*"
          ...
          return (
            <motion.button key={pet.id} whileTap={{ scale: 0.92 }}
              ...
              style={{
                ...
                border: `1.5px solid ${active ? C.purple : 'rgba(255,255,255,0.08)'}`,
                boxShadow: active ? `0 4px 16px ${C.purple}30` : 'none',
                minWidth: 72,
              }}>
              ...
              {active && <div className="w-4 h-1 rounded-full" style={{ background: C.purple }} />}
            </motion.button>
          );
        })}
      </div>
...
// ─── Preset Card com cor e glow visual ───────────────────────────────────────
const PRESET_COLORS = {
  cinematico: '#1e40af',
  magico:     '#7c3aed',
  lifestyle:  '#065f46',
...
function PresetCard({ preset, active, onSelect }) {
  const color = PRESET_COLORS[preset.id] || C.purple;
  return (
...
export default function TutorCatModulePage() {
  ...
  const [tutorUploadPct, setTutorUploadPct] = useState(0);
  const [tutorUrl,       setTutorUrl]       = useState('');

  const catBlobRef   = useRef('');
  const tutorBlobRef = useRef('');

  useEffect(() => () => {
    if (catBlobRef.current)   URL.revokeObjectURL(catBlobRef.current);
    if (tutorBlobRef.current) URL.revokeObjectURL(tutorBlobRef.current);
  }, []);

  const handleCatFile = useCallback(async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    // Reseta o value para permitir re-seleção do mesmo arquivo
  ...
  const handleTutorFile = useCallback(async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const target = e.target;
    setTimeout(() => { target.value = ''; }, 0);
    ...
    if (tutorBlobRef.current) URL.revokeObjectURL(tutorBlobRef.current);
    const blob = URL.createObjectURL(file);
    tutorBlobRef.current = blob;
    setTutorPreview(blob);
    setTutorUrl('');
    setTutorUploading(true);
    setTutorUploadPct(0);
    try {
      const url = await uploadFile(file, setTutorUploadPct);
      setTutorUrl(url);
    } catch {
      setError('Erro ao enviar sua foto. Tente novamente.');
      setTutorPreview('');
      setTutorUrl('');
    } finally {
  ...
  const handleGenerate = async () => {
    ...
    if (!tutorUrl)        { setError('Aguarde o upload da sua foto.'); return; }
    ...
    try {
      ...
      if (!finalUrl) throw new Error(res.data?.message || 'A IA não retornou imagem.');
      ...
      console.error('[TutorCat] generate error:', err);
      setError(err?.response?.data?.message || err?.message || 'Erro ao gerar a montagem.');
    } finally {
  ...
  const handleDownload = () => {
    ...
    a.href = resultUrl;
    a.download = `tutor-cat-${Date.now()}.jpg`;
    a.target = '_blank';
    a.click();
  };

  const canGenerate =
    !!catUrl && !!tutorUrl && !!selectedPet?.id &&
    !isGenerating && !catUploading && !tutorUploading &&
      ...
      <div className="sticky top-0 z-40 px-4 pt-5 pb-3"
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-sm font-black text-white">Tutor + Gato</p>
          </div>
      ...
      <div className="px-4 pt-2 pb-4 space-y-4 max-w-xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10 p-5">
            ...
            <h1 className="text-xl font-black text-white leading-tight mb-0.5">📸 Tutor + Gato</h1>
            <p className="text-[11px] text-white/70 font-medium leading-relaxed max-w-xs">
              Monte cenas mágicas, afetivas ou cinematográficas com você e seu gato juntos.
            </p>
        ...
        <div>
          <p className="text-[9px] font-black uppercase tracking-[3px] text-white/35 mb-3">
            Fotos para a montagem
          </p>
          <div className="grid grid-cols-2 gap-3">
            <UploadSlot
              label="Foto do gato" icon={PawPrint}
              previewUrl={catPreview} onFile={handleCatFile}
              uploading={catUploading} uploadPct={catUploadPct} accent="#10b981"
            />
            <UploadSlot
              label="Sua foto" icon={User2}
              previewUrl={tutorPreview} onFile={handleTutorFile}
              uploading={tutorUploading} uploadPct={tutorUploadPct} accent="#8B4AFF"
            />
          </div>
          ...
          <div className="flex gap-4 mt-2 px-1">
            {[
              { url: catUrl, uploading: catUploading, pct: catUploadPct, label: 'Gato', color: '#10b981' },
              { url: tutorUrl, uploading: tutorUploading, pct: tutorUploadPct, label: 'Sua foto', color: '#8B4AFF' },
            ].map(({ url, uploading, pct, label, color }) => (
              <div key={label} className="flex items-center gap-1.5 flex-1">
                <div className="w-1.5 h-1.5 rounded-full"
                  style={{ background: url ? color : uploading ? '#f59e0b' : 'rgba(255,255,255,0.12)' }} />
                <span className="text-[9px] font-bold"
                  style={{ color: url ? color : 'rgba(255,255,255,0.22)' }}>
                  {url ? `${label} ✓` : uploading ? `${pct}%` : 'Aguardando'}
                </span>
              </div>
            ))}
          </div>
          <p className="text-[9px] text-white/18 font-medium mt-1.5 text-center">
            Use fotos com rosto visível para melhores resultados
          </p>
        </div>

        {/* Presets */}
        <div>
          <p className="text-[9px] font-black uppercase tracking-[3px] text-white/35 mb-2.5">Estilo da cena</p>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={`Ex: nós dois em uma floresta mágica ao anoitecer, estilo ${activePreset.label.toLowerCase()}...`}
        ...
        <motion.button
          ...
          style={{
            ...
            boxShadow: canGenerate ? '0 8px 28px rgba(249,115,22,0.35)' : 'none',
          ...
          <span className="relative z-10 flex items-center justify-center gap-2">
            {isGenerating ? (
              ...
              <><Sparkles size={16} /> Gerar com IA — {effectiveCost} GPTS</>
            )}
          </span>
        </motion.button>
        ...
        <AnimatePresence>
          {resultUrl && (
            <motion.div
              ...
              style={{ border: '1.5px solid rgba(223,255,64,0.22)' }}>
              <div className="relative">
                <img src={resultUrl} alt="Montagem gerada" className="w-full object-cover" />
                <div className="absolute top-3 right-3">
                  <span className="text-[8px] font-black px-2.5 py-1 rounded-full"
                    style={{ background: C.accent, color: '#1a1a00' }}>✓ Gerado</span>
                </div>
              </div>
              <div className="p-4 space-y-2" style={{ background: 'rgba(26,16,48,0.95)' }}>
                <div className="flex items-center gap-1.5 mb-3">
                  <Check size={14} style={{ color: C.accent }} />
                  <p className="text-xs font-black text-white">Montagem criada com sucesso!</p>
                </div>
                <motion.button whileTap={{ scale: 0.96 }} onClick={handlePublish}
                  ...
                  style={{ background: GRADIENT, boxShadow: '0 6px 20px rgba(249,115,22,0.3)' }}>
                  <Globe2 size={15} /> Publicar no Comunigato
                </motion.button>
                <motion.button whileTap={{ scale: 0.96 }} onClick={handleDownload}
                  ...
                  <Download size={13} /> Salvar imagem
                </motion.button>
