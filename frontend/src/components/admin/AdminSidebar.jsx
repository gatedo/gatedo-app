...
import { 
  ...
  Settings, LogOut, ArrowLeft, HeartHandshake, TrendingUp, ChevronLeft, ChevronRight 
} from 'lucide-react';
...
export default function AdminSidebar({ activeTab, setActiveTab, isOpen, onClose }) {
  ...
  const [isCollapsed, setIsCollapsed] = useState(false); // Estado para o Desktop
      ...
      >
        ...
        <div className={`p-4 flex flex-col items-center border-b border-gray-50/10 mb-2 relative`}>
           ...
           <button 
             ...
             className="hidden lg:flex absolute -right-3 top-10 bg-white border border-gray-100 rounded-full p-1 shadow-md text-[#6158ca] hover:scale-110 transition-transform"
           >
        ...
        <nav className={`flex-1 ${isCollapsed ? 'px-2' : 'px-4'} py-4 space-y-2 overflow-y-auto custom-scrollbar`}>
          {MENU_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                {/* Filete Amarelo na lateral quando recolhido e ativo */}
                ...
                <Icon 
                  size={20} 
                  style={{ color: isCollapsed ? (isActive ? COLORS.accent : '#fff') : (isActive ? COLORS.accent : 'gray') }}
        ...
        <div className={`p-4 border-t border-gray-100 space-y-3 ${isCollapsed ? 'bg-transparent' : 'bg-gray-50/50'}`}>
            ...
            {!isCollapsed && (
              <button 
                  onClick={() => { if(window.confirm("Sair do Admin?")) navigate('/home'); }}
