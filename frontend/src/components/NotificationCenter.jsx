...
export default function NotificationCenter({
  ...
  const handleAction = (notif) => {
    ...
    switch (notif.type) {
      case 'MED_REMINDER':
        // Tratamento → perfil do gato na aba Saúde
        if (petId) navigate(`/cat/${petId}?tab=SAUDE`);
        else navigate('/cats');
        break;
      case 'IGENT_ALERT':
      case 'IGENT_PREDICTIVE':
        // Insight IA → perfil do gato na aba Saúde
        if (petId) navigate(`/cat/${petId}?tab=SAUDE`);
        else navigate('/igentvet');
        break;
      case 'VACCINE_DUE':
      case 'VACCINE_OVERDUE':
        // Vacina → aba Imunizantes ou direto no form
        if (petId) navigate(`/cat/${petId}?tab=IMUNIZANTES`);
        else navigate('/cats');
        break;
  ...
  return (
    <motion.div
      ...
      className="fixed bg-white rounded-[24px] shadow-2xl border border-gray-100 overflow-hidden z-[60]"
      style={{ top: 72, right: 16, left: 16, maxWidth: 360, marginLeft: "auto" }}
