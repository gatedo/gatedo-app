...
export class IgentService {
  ...
  private async callAI(
    ...
    try {
      ...
      if (!geminiRes.ok) {
        ...
        this.logger.error(`Gemini REST ${geminiRes.status}: ${errBody}`);
  ...
  // ─── MAPA DE RELEVÂNCIA POR SINTOMA ─────────────────────────────────────────
  private readonly SYMPTOM_RELATED_TYPES: Record<string, string[]> = {
  ...
  // ─── BIBLIOTECA DE PROMPTS POR SINTOMA ─────────────────────────────────────
  private readonly SYMPTOM_PROMPTS: Record<
    string,
    {
      persona: string;
      focus: string;
      redFlags: string[];
      breedRisks: Record<string, string>;
...
SEMPRE checar: pulgas/carrapatos, mudança de ração recente, novo produto no ambiente.`,
      ...
      whenToVet:
        'Consulta presencial em até 48h se houver lesões com crostas, sangramento ou expansão rápida. Em 7 dias se for apenas prurido leve sem lesão visível.',
    },
...
Terceira pálpebra visível? Estado vacinal (raiva, cálicivírus, herpes incluídos na polivalente)?`,
      ...
      whenToVet:
        'URGENTE (24h) se olho fechado com blefarospasmo, opacidade na córnea ou secreção intensa. Eletivo (48-72h) para secreção leve ou epífora sem outros sinais.',
    },
...
Histórico de otites? Outros animais em casa (Otodectes é altamente contagioso)?`,
      ...
      whenToVet:
        'URGENTE se inclinação de cabeça com perda de equilíbrio. Em 48h se houver secreção abundante com odor ou otohematoma. Eletivo (7 dias) para coceira leve sem secreção.',
    },
...
Mudanças no padrão de sono, alimentação, uso da caixa de areia?`,
      ...
      whenToVet:
        'URGENTE se convulsão, ataxia ou agressividade extrema súbita. Em 48-72h se automutilação ou parada total de alimentação. Eletivo para ansiedade/comportamento sem sinais físicos.',
    },
...
Gato jovem com diarreia → pensar parasitas. Idoso → IBD/linfoma.`,
      ...
      whenToVet:
        'URGENTE (imediato) se sangue, distensão abdominal, prostração intensa ou gato obeso sem comer. Em 24h se vômitos/diarreia > 4x sem sangue. Eletivo se 1-2 episódios isolados com gato ativo.',
    },
...
Bebe mais água que o habitual? Histórico de cálculos?`,
      ...
      whenToVet:
        'EMERGÊNCIA IMEDIATA se macho sem urinar. URGENTE (6h) se hematúria + prostração. Em 24h para qualquer alteração urinária em macho. Eletivo (48-72h) para fêmea com xixi fora da caixa sem outros sinais.',
    },
...
Em gato idoso: rigidez matinal, dificuldade de pular?`,
      ...
      whenToVet:
        'EMERGÊNCIA IMEDIATA se paralisia súbita de posteriores (suspeita de TEA). URGENTE (2h) pós-trauma com membro não apoiado. Em 24h para coxeamento leve sem trauma. Eletivo para rigidez gradual em idosos.',
    },
...
Histórico de exposição a toxinas?`,
      ...
      whenToVet:
        'EMERGÊNCIA IMEDIATA na grande maioria dos casos nesta categoria. Vá ao veterinário ou emergência 24h AGORA se houver qualquer sinal dos red flags acima.',
    },
  };

  // ─── MONTA PERSONA E INSTRUÇÕES CLÍNICAS ───────────────────────────────────
  private buildSymptomPrompt(symptomId: string, pet: any, symptomLabel: string): string {
    ...
    const breedNote = sp.breedRisks[breed]
      ...
      : breed !== 'SRD'
        ? `Raça ${breed} — verificar predisposições específicas.`
        : '';
...
`.trim();
  ...
  private buildFocusedContext(
    ...
    const allRecords: any[] = clinicalContext?.healthRecords || pet.healthRecords || [];
    const now = new Date();
    ...
    const vaccines =
      allRecords
        ...
        .slice(0, 5)
        .map((r) => {
          const next = r.nextDueDate
            ? ` | Proxima: ${new Date(r.nextDueDate).toLocaleDateString('pt-BR')}`
            : '';
          const overdue =
            r.nextDueDate && new Date(r.nextDueDate) < now ? ' [VENCIDA]' : '';
    ...
    const dietParts = [
      ...
      pet.foodType?.length ? `tipo: ${(pet.foodType as string[]).join('/')}` : null,
    ].filter(Boolean);
    ...
    const personality = pet.personality?.length
      ? (pet.personality as string[]).join(', ')
      : null;
    ...
    const clinicalHistory =
      allRecords
        ...
        .map(
          (r) =>
            `  - ${r.title || (r.notes || '').substring(0, 70)} (${new Date(r.date).toLocaleDateString('pt-BR')})`,
        )
        .join('\n') ||
...
8. Ao final de respostas complexas, faca 1 pergunta de acompanhamento clinico relevante`.trim();
  ...
  // ─── MÉTODO 3: GERAR DADOS DO PRONTUÁRIO ────────────────────────────────────
  ...
  // ─── MÉTODO 5: LISTAR SESSÕES DE UM PET ─────────────────────────────────────
  async getSessions(petId: string) {
  ...
  // ─── MÉTODO 6: ATUALIZA CONTEXTO DA IA ──────────────────────────────────────
