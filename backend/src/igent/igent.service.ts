...
export class IgentService {
  ...
  // Garante que herpes ocular NÃO aparece numa consulta de vômito
  ...
  // Cada sintoma tem: persona clínica especializada + perguntas de triagem + red flags
  private readonly SYMPTOM_PROMPTS: Record<string, {
    persona:    string;   // expertise específica do iGentVet para este sintoma
    focus:      string;   // o que priorizar na análise
    redFlags:   string[]; // sinais que tornam isUrgent = true
    breedRisks: Record<string, string>; // riscos específicos por raça
    triageQuestions: string[]; // 3 perguntas de triagem para o tutor
    whenToVet:  string;   // orientação sobre urgência presencial
  }> = {
...
Histórico de exposição a toxinas?`,
  ...
  // ─── MONTA PERSONA E INSTRUÇÕES CLÍNICAS para o prompt de análise ────────────
...
`.trim();
  ...
  // ─── HELPER: Monta contexto clínico COMPLETO ────────────────────────────────

  // Dados basais (vacinas, meds, dieta) SEMPRE presentes — nunca filtrados por sintoma
  // Histórico clínico filtrado pelo sintoma para manter foco
  private buildFocusedContext(
    ...
    // ── 2. MEDICACOES ATIVAS (90 dias) — sempre presente ─────────────────────
    ...
    const docs = (pet.documents || []).slice(0, 3)
      .map((d: any) => `  - ${d.name} (${new Date(d.date).toLocaleDateString('pt-BR')})`)
      .join('\n') || null;
    ...
    const personality = pet.personality?.length
      ? (pet.personality as string[]).join(', ') : null;

    // ── 5. HISTORICO CLINICO FILTRADO pelo sintoma ────────────────────────────
    const relatedTypes = this.SYMPTOM_RELATED_TYPES[symptomId] || ['GENERAL'];
    const clinicalHistory = allRecords
      .filter((r) => {
        const type = (r.type || '').toUpperCase();
        if (['VACCINE','MEDICATION','MEDICINE','VERMIFUGE','PARASITE'].includes(type)) return false;
...
8. Ao final de respostas complexas, faca 1 pergunta de acompanhamento clinico relevante`.trim();
  ...
  // ─── MÉTODO 3: GERAR DADOS DO PRONTUÁRIO (frontend gera o PDF) ───────────────
