...
async function ensureTutorMeta(userId: string, points = 300, credits = 300) {
  await safeExec(`TutorPoints ${userId}`, () =>
    prisma.tutorPoints.upsert({
      where: { userId },
      update: {
        points,
        totalEarned: points,
  ...
  await safeExec(`UserCredits ${userId}`, () =>
...
async function createNoticeIfMissing(prisma: any, notice: {
  title: string
  content: string
  type?: string
  isActive?: boolean
...
async function main() {
  ...
  await createNoticeIfMissing(prisma, {
    ...
    content:
      'A comunidade do GATEDO está em fase beta. Algumas funções podem evoluir nas próximas atualizações.',
    type: 'UPDATE',
    xpReward: 3,
  })
  ...
  await createNoticeIfMissing(prisma, {
    ...
    content:
      'Agora você pode ganhar XP ao interagir com comunicados oficiais dentro do app.',
    type: 'INFO',
    xpReward: 3,
  })
  ...
  await createNoticeIfMissing(prisma, {
    ...
    content:
      'Você faz parte dos primeiros tutores do GATEDO. Explore a comunidade.',
    type: 'UPDATE',
    xpReward: 3,
  })
  ...
  await createNoticeIfMissing(prisma, {
    ...
    content:
      'Use o Studio, publique no Comunigato e fortaleça seu perfil.',
    type: 'INFO',
    xpReward: 3,
  })
  ...
  await createNoticeIfMissing(prisma, {
    ...
    content:
      'Use o marcador dos posts para montar sua coleção de referências.',
    type: 'INFO',
    xpReward: 2,
  })
